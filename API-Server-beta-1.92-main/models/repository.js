...
export default class Repository {
    ...
    getAll(HttpContext) {
        ...
        for (let i = 0; i < Object.keys(HttpContext.path.params).length; i++) {
            if (Object.keys(HttpContext.path.params)[i] != "limit" && 
                Object.keys(HttpContext.path.params)[i] != "offset" &&
                Object.keys(HttpContext.path.params)[i] != "fields" && 
                Object.keys(HttpContext.path.params)[i] != "sort") 
            {
                filter = true;
            }
        }
                    ...
                    for (let i = 0; i < Object.keys(HttpContext.path.params).length; i++) {
                            ...
                            Object.keys(HttpContext.path.params)[i] != "sort") 
                        {
                            ...
                            if(HttpContext.path.params[Object.keys(HttpContext.path.params)[i]].includes('*')){
                                if(HttpContext.path.params[Object.keys(HttpContext.path.params)[i]].startsWith("*") && 
                                   HttpContext.path.params[Object.keys(HttpContext.path.params)[i]].endsWith("*")){
                                   let valeur = HttpContext.path.params[Object.keys(HttpContext.path.params)[i]].replaceAll("*", "");
                                    if(typeof(valeur) == String && type0f(data[Object.keys(HttpContext.path.params)[i]]) ==String){
                                        if (data[Object.keys(HttpContext.path.params)[i]].toUpperCase().includes(valeur.toUpperCase())) {
                                            dataBind.push(this.model.bindExtraData(data));
                                        }
                                    }else{
                                        if (data[Object.keys(HttpContext.path.params)[i]].includes(valeur)) {
                                            dataBind.push(this.model.bindExtraData(data));
                                        }
                                    }
                                }
                                else if(HttpContext.path.params[Object.keys(HttpContext.path.params)[i]].startsWith("*")){
                                    let valeur = HttpContext.path.params[Object.keys(HttpContext.path.params)[i]].replace("*", "");

                                    if(typeof(valeur) == String && type0f(data[Object.keys(HttpContext.path.params)[i]]) ==String){
                                        if (data[Object.keys(HttpContext.path.params)[i]].toUpperCase().startsWith(valeur.toUpperCase())) {
                                            dataBind.push(this.model.bindExtraData(data));
                                        }
                                    }
                                    else{
                                        if (data[Object.keys(HttpContext.path.params)[i]].startsWith(valeur)) {
                                            dataBind.push(this.model.bindExtraData(data));
                                        }
                                    }
                                }
                                    ...
                                    if(typeof(valeur) == String && type0f(data[Object.keys(HttpContext.path.params)[i]]) ==String){
                                        if (data[Object.keys(HttpContext.path.params)[i]].toUpperCase().endsWith(valeur.toUpperCase())) {
                                            dataBind.push(this.model.bindExtraData(data));
                                        }
                                    }
                                    else{
                                        if (data[Object.keys(HttpContext.path.params)[i]].endsWith(valeur)) {
                                            dataBind.push(this.model.bindExtraData(data));
                                        }
                                    }
        ...
        let allFields = Object.keys(dataBind[0]);
         dataBind = dataBind.filter(element => {
          let info ="";
          for(let i=0;i<allFields.length;i++){
            info =info+element[allFields[i]];
          }
          const duplicate = vue.has(info);
          vue.add(info);
          return !duplicate;
        });
        dataBind = CollectionFilter.Filter(dataBind, HttpContext);
        return dataBind;
    }
    get(id) {
        for (let object of this.objects()) {
    ...
    findByField(fieldName, valeur, excludedId = 0) {
        if (fieldName) {
            let index = 0;
            for (let object of this.objects()) {
                try {
                    if (object[fieldName] === valeur) {
